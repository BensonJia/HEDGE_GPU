...
def _update_paths(prm: dict, run_config: dict) \
        -> dict:
    prm["save_hedge"] = Path("data").resolve() / "hedge_inputs" / f"{run_id(run_config)}"
    ...
    prm["save_other"] = Path("data").resolve() / "other_outputs" / f"{run_id(run_config)}"
...
def _make_dirs(prm: dict):
    for folder in ["hedge_inputs", "other_outputs"]:
        if not os.path.exists(Path("data").resolve() / folder):
            os.mkdir(Path("data").resolve() / folder)
