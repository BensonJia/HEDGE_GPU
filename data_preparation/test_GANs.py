...
def generate_synthetic_data(n_train_data, generators, ks, params):
    profiles = {}
    for k in ks:
        n_batch = max(int(n_train_data[k] / 50) + 1, 2)
        profiles_nonzero = generators[k](
            th.randn(n_batch, 1,device="cpu" if not th.cuda.is_available() else "cuda")
        ).view(-1, params[k]['n_profile']).cpu().detach().numpy()[:n_train_data[k]]
