...
def filter_validity(
    ...
    # print(not (data_type == 'gen' and prm['gen_CLNR']))
    if not (data_type == 'gen' and prm['gen_CLNR']):
    ...
    #     if var_label not in row:
    #         raise ValueError(f"Column '{var_label}' is missing in the DataFrame.")

    #     if pd.isna(row[var_label]):  # 兼容 None 和 NaN
    #         return False

    #     return row.get("keep", False)  # 如果 'keep' 不存在，返回 False

    def _remove_none(row):
        if row[var_label] is None or np.isnan(row[var_label]):
            return False

        return row.get("keep", True)
    print(data.columns)
