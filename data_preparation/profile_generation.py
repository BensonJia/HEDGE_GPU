...
class GAN_Trainer():
    def __init__(self, training_params, prm):
        ...
        self.prm = prm
        if tensorboard:
            log_dir = self.prm['save_other'] / 'tensorboard'
    ...
    #     self.outputs = th.tensor(outputs, device=device)
    #     if outputs_test is not None:
    #         self.outputs_test = th.tensor(outputs_test,device=device)
    ...
    #     self.mean_real_output = np.nanmean(self.outputs.cpu().numpy())
    #     self.std_real_output = np.nanstd(self.outputs.cpu().numpy())
    #     self.mean_test_output = np.nanmean(self.outputs_test.cpu().numpy())
    #     self.std_test_output = np.nanstd(self.outputs_test.cpu().numpy())
    ...
    def torch_nanstd(x, dim=None, unbiased=True, keepdim=False):
        # Mask NaNs
        print(type(x))
        mask = th.isnan(x)
        valid_values = x[~mask]
        ...
        return th.std(valid_values, unbiased=unbiased)
    
    def add_training_data(
        self, inputs=None, outputs=None, outputs_test=None,
        p_clus=None, p_trans=None, n_clus=None, plot=True
    ):
        # Move outputs to GPU if available
        self.train_data_length = len(outputs)
        self.outputs = th.tensor(outputs, device=device, dtype=th.float32)
        
        if outputs_test is not None:
            self.outputs_test = th.tensor(outputs_test, device=device, dtype=th.float32)

        # Efficient normalization (vectorized)
        if self.normalised and inputs is not None:
            inputs = th.tensor(inputs, device=device, dtype=th.float32)
            divisor = inputs[:, 0].unsqueeze(-1)  # Extract first column for division

            self.inputs = inputs / divisor
            self.outputs = self.outputs / divisor
            if outputs_test is not None:
                self.outputs_test = self.outputs_test / divisor

        if plot:
            self.plot_inputs()

        # Compute statistics using PyTorch (avoid CPU-GPU transfers)
        self.mean_real_output = th.nanmean(self.outputs).item()
        output_val = self.outputs
        mask = th.isnan(output_val)
        valid_values = output_val[~mask]
        self.std_real_output = th.std(valid_values, unbiased=True).item()

        
        if outputs_test is not None:
            self.mean_test_output = th.nanmean(self.outputs_test).item()
            output_val = self.outputs
            mask = th.isnan(output_val)
            valid_values = output_val[~mask]
            self.std_real_output = th.std(valid_values, unbiased=True).item()
        ...
        self.train_data = self.outputs
        self.test_data = self.outputs_test if outputs_test is not None else None
    ...
    def plot_inputs(self):
        if self.prm['plots']:
            ...
            for i in range(min(20, len(self.outputs))):
                plt.plot(self.outputs[i].cpu().numpy())
    ...
    def get_train_loader(self):
        ...
        print(f"Dataset size: {len(train_data_not_nan)}")  # Ensure dataset is not empty
        num_samples = train_data_not_nan.size(0)

        # 计算需要填充的行数
        pad_size = (self.n_items_generated - num_samples) if num_samples < self.n_items_generated else 0

        if pad_size > 0:
            pad = th.zeros((pad_size, train_data_not_nan.size(1)), dtype=train_data_not_nan.dtype)
            train_data_not_nan = th.cat([train_data_not_nan, pad], dim=0)

        num_samples = train_data_not_nan.size(0)  # 获取数据样本数
        remainder = num_samples % self.n_items_generated

        if remainder > 0:
            train_data_cut = train_data_not_nan[: -remainder]  # 只保留可整除部分
        else:
            train_data_cut = train_data_not_nan  # 直接使用原数据
        train_data_cut = train_data_cut.view(
            (-1, train_data_cut.size()[1] * self.n_items_generated)
        )
        
        print(f"Dataset size: {len(train_data_cut)}")  # Ensure dataset is not empty
    ...
    def split_inputs_and_outputs(self, train_data):
        real_inputs = th.randn(self.batch_size_, self.dim_latent_noise).to(device)
        real_outputs = train_data.to(device)
    ...
    def initialise_generator_and_discriminator(self):
        ...
        self.discriminator = Discriminator(self).to(device)
    ...
    def train_discriminator(self, real_inputs, real_outputs, test):
        generated_outputs = self.generator(real_inputs, test=test).to(device)
        generated_samples_labels = th.zeros((self.batch_size_, 1)).to(device)
        ...
        all_samples = th.cat((real_samples, generated_samples)).to(device)
        ...
        output_discriminator = self.discriminator(all_samples.to(th.float32)).to(device)
        loss_discriminator = self.loss_function(output_discriminator, all_samples_labels)
        loss_discriminator.backward()
        self.optimizer_discriminator.step()
        if tensorboard:
            self.writer.add_scalar('Loss/Discriminator', loss_discriminator.item(), self.global_step)  # Tensorboard

        return loss_discriminator

    def get_real_samples_labels(self):
        return th.ones((self.batch_size_, 1)).to(device)
    ...
    def _compute_statistical_indicators_generated_profiles(self, generated_samples):
        ...
        percentiles_generated['mean'] = th.mean(generated_samples_2d, dim=0)
    ...
    def train_generator(self, real_inputs, final_n, epoch, test, plot):
        ...
        percentiles_generated = self._compute_statistical_indicators_generated_profiles(generated_samples)
        ...
        for key in [f'p{percentile}' for percentile in self.percentiles] + ['mean']:
            episode['loss_percentiles'] += th.sum(
                th.square(
                    ...
                    - th.tensor(self.percentiles_inputs_train[self.k][key][~self.zero_values], device=device)
                )
            )
        ...
        if tensorboard:
            self.writer.add_scalar('Loss/Generator', episode['loss_generator'].item(), self.global_step)                # Tensorboard
            self.writer.add_scalar('Metrics/MeanError1', episode['mean_err_1'].item(), self.global_step)                # Tensorboard
            self.writer.add_scalar('Metrics/StdError1', episode['std_err_1'].item(), self.global_step)                  # Tensorboard
            self.writer.add_scalar('Metrics/ShareLargeError1', episode['share_large_err_1'].item(), self.global_step)   # Tensorboard
            ...
            for color, percentiles_, label in zip(
                ...
                for indicator in [f'p{percentile}' for percentile in self.percentiles] + ['mean']:
                    percentiles_np = percentiles_[indicator] if label == 'original' \
                        else percentiles_[indicator].cpu().detach().numpy()
                    if label == 'generated':
    ...
    def plot_metrics_over_time(self, episodes, epoch, test=False):
        ...
        for label, x, y in zip(
            ...
            axs[x, y].plot(episodes[label][:epoch].detach().cpu().numpy())
    ...
    def plot_final_hist_generated_vs_real(self, generated_outputs, real_outputs, epoch):
        generated_outputs = generated_outputs.cpu().detach().numpy()
        ...
        if self.prm['plots']:
            ...
            real_outputs_reshaped = np.array(real_outputs.cpu().detach().numpy(), dtype=int).flatten()
    ...
    def train(self, plot=True):
        ...
        for potential_path in potential_paths:
            ...
            if self.recover_weights and all(os.path.exists(file) for file in files.values()):
                ...
                offset_idx = 0
        idx = 0
        self.global_step = 0  # Tensorboard
        for epoch in tqdm(range(self.n_epochs)):
            ...
            for n, train_data in enumerate(self.train_loader):
                ...
                with th.no_grad():
                    ...
                    for key in episode:
                        ...
                        episodes[key][idx + offset_idx] = episode[key]
                    if test:
                        # print(episodes_test['loss_discriminator'])
                        if epoch_test < len(episodes_test['loss_discriminator']):
                            episodes_test['loss_discriminator'][epoch_test] = loss_discriminator
                            for key in episode:
                                episodes_test[key][epoch_test] = episode[key]
                        if self.data_type == 'car':
            ...
            if episodes['loss_percentiles'][epoch] < self.tol_loss_percentiles:
                ...
                break
            print(f"\nEpoch [{epoch}/{self.n_epochs}], Loss D: {loss_discriminator.item()}, Loss G: {episode['loss_generator'].item()}")
...
class Generator(nn.Module):
    ...
    def _initialise_model(self, gan_trainer):
        ...
        if self.nn_type == 'linear':
            self.model = nn.Sequential(
                ...
                nn.InstanceNorm1d(16 * multiplier, affine=False),  # Replace BatchNorm1d with InstanceNorm1d
                nn.ReLU(),
                nn.Dropout(self.dropout),  

                nn.Linear(16 * multiplier, 32 * multiplier),
                nn.InstanceNorm1d(32 * multiplier, affine=False),  # Replace BatchNorm1d with InstanceNorm1d
                nn.ReLU(),

                nn.Linear(32 * multiplier, 64 * multiplier),
                nn.InstanceNorm1d(64 * multiplier, affine=False),  # Replace BatchNorm1d with InstanceNorm1d
                nn.ReLU(),
                nn.Dropout(self.dropout),  # 在较深层使用 Dropout

                nn.Linear(64 * multiplier, 32 * multiplier),
                nn.InstanceNorm1d(32 * multiplier, affine= False),  # Replace BatchNorm1d with InstanceNorm1d
                nn.ReLU(),

                nn.Linear(32 * multiplier, self.size_output)  # **无 Sigmoid**，使用 BCEWithLogitsLoss
            )
            ...
            self.conv = nn.Sequential(
                ...
                nn.InstanceNorm1d(self.size_output * 4),  # Replace BatchNorm1d with InstanceNorm1d
                nn.ReLU(),
                nn.Dropout(0.3),
                nn.ConvTranspose1d(
                   self.size_output * 4, self.size_output * 2, kernel_size=3, stride=2, padding=1,
                ),
                nn.InstanceNorm1d(self.size_output * 2),  # Replace BatchNorm1d with InstanceNorm1d
                nn.ReLU(),
                nn.Dropout(0.3),
                nn.ConvTranspose1d(
                   self.size_output * 2, self.size_output, kernel_size=3, stride=2, padding=1
                ),
                nn.InstanceNorm1d(self.size_output),  # Replace BatchNorm1d with InstanceNorm1d
                nn.ReLU(),
            )
    ...
    def forward(self, x, test=False):
        ...
        if self.data_type != 'car':
            output = output.to(device)
            noise = noise.to(device)
            output = th.clamp(output + noise,  min=th.tensor(self.min, device=device),max=th.tensor(self.max * 1.1).to(device))
            # output = th.clamp(output + noise, min=self.min, max=self.max * 1.1)
        else:
            output = th.clamp(output + noise, min=th.tensor(self.min, device=device))
...
def compute_profile_generators(
    ...
    params = {
        'profiles': True,
        'batch_size': 131072,
        'n_epochs': int(5e5 / len(profiles_train)),
    ...
    if data_type == 'gen':
        ...
        params['lr_start'] = 1e-1       # 1e-3
        params['lr_end'] = 1e-4         # 1e-1
        params['initial_noise'] = 0.01
        params['n_epochs_initial_noise'] = 50
